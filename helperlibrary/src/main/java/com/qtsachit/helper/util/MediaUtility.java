...
public class MediaUtility {
        ...
        try {
            if (cur != null && cur.getCount() > 0) {
                while (cur.moveToNext()) {
                    ...
                    if (duration == 0)
                        LoggerUtility.error(TAG, " The image size was found to be 0. Reason: UNKNOWN");

                }    // end while
            } else if (cur.getCount() == 0) {
                LoggerUtility.error(TAG, " cur size is 0. File may not exist");
            } else {
                LoggerUtility.error(TAG, " cur is null");
            }
        } finally {
    ...
    public static long getMediaSize(Context ctx, Uri mediaUri) {
        ...
        long size = -1;
        if (!MediaUtility.isMediaContentUri(mediaUri)) {
            LoggerUtility.error(TAG, " Not a valid content uri");
            return size;
        }
        ...
        try {
            if (cur != null && cur.getCount() > 0) {
                while (cur.moveToNext()) {
                    ...
                    if (size == 0) {
                        LoggerUtility.error(TAG, " The media size was found to be 0. Reason: UNKNOWN");
                    }
                } // end while
            } else if (cur.getCount() == 0) {
                LoggerUtility.error(TAG, " cur size is 0. File may not exist");
            ...
            if (cur != null && cur.getCount() > 0) {
                while (cur.moveToNext()) {
                    ...
                    if (TextUtils.isEmpty(colName)) {
                        LoggerUtility.error(TAG, " The file name is blank or null. Reason: UNKNOWN");
                    }

                } // end while
            } else if (cur != null && cur.getCount() == 0) {
                LoggerUtility.error(TAG, "  File may not exist");
            } else {
                LoggerUtility.error(TAG, " cur is null");
            }
        } finally {
